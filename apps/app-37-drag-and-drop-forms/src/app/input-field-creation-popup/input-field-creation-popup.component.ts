...
export class InputCreationPopupComponent {
            ...
            for (let i = 0; i < this.numberOfValidation; i++) {
                if (this.validators[i].validationName === 'required') {
                    ...
                    const messageString: string =
                        (('Please enter less than ' + this.validators[i].maxLength) as string) +
                        ' characters';
                    validations.push({
                        validationName: this.validators[i].validationName,
                    ...
                    const messageString: string =
                        (('Please enter more than ' + this.validators[i].minLength) as string) +
                        ' characters';
                    validations.push({
                        validationName: this.validators[i].validationName,
                        message: messageString,
    ...
    withLayoutFormCreation(): void {
        ...
        const fieldIndex: number = this.jsonStorage
            .getAllFields()
            .findIndex(item => item.rowId === this.data.rowId);
        const fieldData: FormcontrolInterface = this.jsonStorage
            .getAllFields()
            .find(item => item.rowId === this.data.rowId) as FormcontrolInterface;
        const layoutIndex: number = fieldData.layout?.findIndex(
            item => item.columnNumber === this.data.columnId,
        ) as number;
        if (this.validatorSelector === false) {
            field = {
